...
- ``history_YYY`` - add fields for specific diagnostic purposes to the
  default output.  For the complete listing go to the `namelist page <http://www.cesm.ucar.edu/models/cesm2/settings/2.2.0/cam_nml.html>`_ and search for namelist variables with the ``history_`` prefix (i.e. ``history_amwg``, ``history_clubb``, etc.)
