...
- **CAM_CONFIG_OPTS**:  The settings in this variable are passed directly to CAM's configure command.  Modifications in this variable change the way CAM is compiled.  The list of possible options are detailed at :ref:`arguments to configure<arguments-to-configure>`.  It is important to note that CAM compsets already have this variable set and that a user will most likely want to append flags as opposed to replacing them.  If the append flag is not used, this variable is reset to only include the values specified and all preset values are removed.  More details on changing CAM's configuration can be found at :ref:`customizing compsets<customizing-compsets>`.
